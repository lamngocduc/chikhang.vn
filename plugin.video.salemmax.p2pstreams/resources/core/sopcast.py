...
def sopstreams(name,iconimage,sop,geturl=False):
	...
	if not xbmc.getCondVisibility('system.platform.windows'):
	    if xbmc.getCondVisibility('System.Platform.Android') or settings.getSetting('force_android') == "true":
			...
			if geturl:return sopstreams_builtin(name,iconimage,sop,True)
            ...
            if config == False: mensagemok(translate(40000),translate(40180),translate(40181), translate(40182))
				...
				if geturl:return [windows_sop_url,False]
...
def sopstreams_builtin(name,iconimage,sop,geturl=False):
	try:
		global spsc
